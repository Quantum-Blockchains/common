...
import { mldsa44Verify } from '../mldsa44/verify.js';
...
const VERIFIERS: Verifier[] = [
  ...
  ['mldsa44', mldsa44Verify],
  ...VERIFIERS_ECDSA
];

const CRYPTO_TYPES: ('ed25519' | 'sr25519' | 'ecdsa' | 'dilithium2' | 'mldsa44')[] = ['ed25519', 'sr25519', 'ecdsa', 'dilithium2', 'mldsa44'];
...
function verifyMultisig (result: VerifyResult, { message, publicKey, signature }: VerifyInput): VerifyResult {
  ...
  try {
    result.isValid = {
      ...
      mldsa44: () => mldsa44Verify(message, signature.subarray(1), publicKey),
    }[type]();
  } catch {
